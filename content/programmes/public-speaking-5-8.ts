...
const playgroundActivities: Record<string, CurriculumActivity> = {
  ...
  "tale-trail": {
    ...
    difficultyLevels: [
      ...
      { level: "Hard", description: "Cards drawn randomly face-down." },
    ],
    debriefPrompts: [],
    type: "physical-game",
  },
  shuffle: {
    ...
    howToPlay:
      "Divide children into Performing Team (2–3 children) and Questioning Team. The Performing Team draws one story — 6 cards — and lays them face-up in the story's printed order. They have preparation time to build a story using the cards in that order. They perform — roleplay and narration — within 2–3 minutes. Questioning team asks questions using their Question Bands. One player summarises in 2–3 sentences. Roles rotate.",
    materials: [
  ...
  "body-talk": {
    ...
    howToPlay:
      "Each child draws a card from the expression / gesture deck and incorporates that expression or gesture into a sentence they speak. The group watches and calls out the feeling or meaning they see. The round continues until every child has completed their set — 4 expressions and 4 gestures each.",
    variations: [
      {
    ...
    difficultyLevels: [
      ...
      { level: "Hard", description: "The group picks the card for the speaker as an added challenge — the speaker has to speak as per that gesture or expression without choosing it themselves." },
    ],
    debriefPrompts: [],
    type: "physical-game",
  },
  ...
  "train-of-thoughts": {
    ...
    difficultyLevels: [
      { level: "Easy", description: "Teacher reads each card aloud, child completes one task from each deck." },
      { level: "Medium", description: "Child reads independently, completes all three tasks in sequence without pausing." },
      { level: "Hard", description: "Child must connect all three task responses into one continuous flowing response." },
    ],
    debriefPrompts: [
      {
  ...
  psychiatrist: {
    ...
    difficultyLevels: [
      ...
      { level: "Hard", description: "Group secretly changes the rule partway through, questioner must notice the shift and identify both rules." },
    ],
    debriefPrompts: [
      {
    ...
    debriefPrompts: [
      {
        ...
        connect:
          "In a real conversation, when do you hear someone say the opposite of what they actually think? How do you tell?",
      },
    ],
    type: "facilitated",
  },
...
const showtimeActivities: Record<string, CurriculumActivity> = {
  "whacky-news": {
    ...
    howToPlay:
      "One child draws a news prompt card. 1 minute to prepare. Child delivers their report standing in front of the class — a clear opening (who they are and what happened), a middle (details), and a closing (what happens next). After the report, each child asks the reporter one question. Reporter answers each in one sentence. New child draws the next card.",
    materials: ["Whacky News Reporter prompt cards (30+ laminated in a flip-top box)"],
    difficultyLevels: [
      { level: "Easy", description: "1 minute prep, teacher reads the prompt aloud." },
      { level: "Medium", description: "30 seconds prep, child reads independently." },
      { level: "Hard", description: "No prep time, child begins immediately on seeing the card." },
    ],
    debriefPrompts: [
      {
        notice: "which report had the clearest ending rather than just stopping.",
        name: "that is a closing that lands.",
        connect: "What made the audience want to ask questions? What does a strong opening do to make the audience ready to listen?",
      },
    ],
    type: "physical-game",
  },
  "mad-ad": {
    ...
    howToPlay:
      "One child receives today's topic from the app. They invent a product connected to that topic and deliver a pitch in front of the class. 1 minute to prepare. Class decides: buying it or not buying it, plus one question. Performer answers in character. Ad structure: Hook — start with a question or surprise · Product name · One amazing reason to buy it · The big ask: \"So what are you waiting for? Get yours today!\"",
    materials: ["App for topic prompts"],
    difficultyLevels: [
      { level: "Easy", description: "1 minute prep, teacher provides the hook opener: \"Start with 'Have you ever...'\"" },
      { level: "Medium", description: "30 seconds, child structures independently." },
      { level: "Hard", description: "No prep, must include at least two deliberate changes of volume or energy." },
    ],
    debriefPrompts: [
      {
        notice: "which pitch made the class lean forward.",
        name: "that is a hook.",
        connect: "What did the hook do to your attention? What happens at the start of a good talk that does the same thing?",
      },
    ],
    type: "facilitated",
  },
  "experience-share": {
    ...
    debriefPrompts: [
      {
        notice: "which share made you feel like you were actually there.",
        name: "that is specific detail doing its work.",
        connect: "What did that person say that made it feel real? What detail could you add to your own stories that does the same thing?",
      },
    ],
    type: "facilitated",
  },
  "magic-box": {
    ...
    howToPlay:
      "Each child reaches into the Magic Box without looking and draws one object. 1 minute to think. Children tell their stories in turn. Group discusses structure after all stories are told. Harder variation: Child draws two objects. Both must appear — one at the beginning, one at the end.",
    materials: [
      "Magic Box (decorated)",
    ...
    debriefPrompts: [
      {
        notice: "which story had the most surprising ending rather than just stopping.",
        name: "that is an unexpected turn.",
        connect: "What makes an ending feel finished rather than just stopped? What is the difference?",
      },
    ],
    type: "physical-game",
  },
  "story-spine": {
    ...
    title: "story spine",
    setupLine:
      "We are building one story together. Each of you has one line. When I point to you, say your line and continue from where the last person ended.",
    howToPlay:
      "Teacher assigns each child one line of the Story Spine. Group uses body and face to react to what is said by each person. Audience names one moment where the story came alive. The Story Spine structure: \"Once upon a time...\" (Child 1) · \"Every day...\" (Child 2) · \"Until one day...\" (Child 3) · \"Because of that...\" (Child 4) · \"Until finally...\" (Child 5) · \"And ever since then...\" (Child 6). For smaller groups, one child can take two lines.",
    materials: [],
    difficultyLevels: [
      { level: "Easy", description: "Teacher provides a character and opening before the game." },
      { level: "Medium", description: "Group decides opening and character in 1 minute first." },
      { level: "Hard", description: "No preparation, first child begins immediately." },
    ],
    debriefPrompts: [
      {
        notice: "which line changed the whole direction of the story.",
        name: "that is a pivot.",
        connect: "How did you know when to change direction in the story? What tells you it is time to do that in a real talk?",
      },
    ],
    type: "facilitated",
  },
  "superhero-sales": {
    ...
    howToPlay:
      "One child draws a superpower prompt from the app. 30 seconds to prepare. Child pitches their superpower to the class. The group votes — useful or useless. Child responds to one challenge question from the group.",
...
const signOffActivities: Record<string, CurriculumActivity> = {
  "tell-the-story": {
    ...
    debriefPrompts: [
      {
        notice: "which story made you feel like you were actually there.",
        name: "that is specific detail.",
        connect: "What did that person say that made it vivid?",
      },
    ],
    type: "facilitated",
  },
  "step-inside": {
    ...
    debriefPrompts: [
      {
        notice: "which character felt most alive.",
        name: "that is commitment to a role.",
        connect: "What did they change about their voice or body to become someone else?",
      },
    ],
    type: "facilitated",
  },
  "show-and-tell": {
    ...
    debriefPrompts: [
      {
        notice: "which presentation made you want to know more.",
        name: "that is enthusiasm landing.",
        connect: "What made you lean in?",
      },
    ],
    type: "facilitated",
  },
  "ask-me": {
    ...
    debriefPrompts: [
      {
        notice: "which follow-up question opened up something new.",
        name: "that is active listening turning into a real question.",
        connect: "What did the interviewer hear that made them ask that?",
      },
    ],
    type: "facilitated",
  },
